...
public static class StatsCollector
{
    ...
    public static MatchStatsSnapshot? Collect(CCSPlayerController? player, OpenSession session, ILogger logger)
    {
        ...
        try
        {
            ...
            snapshot.Mvps = player.MVPs;
            snapshot.Team = player.TeamNum;
        ...
        {
            // Контроллер уже мог стать невалидным — это штатный исход на выходе игрока.
    ...
    public static int ReadPing(CCSPlayerController player)
    {
        try
        {
