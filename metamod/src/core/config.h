...
// Каталог, из которого плагин читает настройки, по абсолютному пути игрового
// каталога (csgo).
//
// Обычное место — addons/configs/ConnectHistory: так раскладывают конфиги
// большинство нативных плагинов CS2, и владельцу сервера не приходится помнить
// отдельное правило для каждого.
//
// До 3.0.2 плагин читал addons/ConnectHistory/configs. Если там лежит
// Settings.json, ВЫИГРЫВАЕТ он, а не новое место: молча перейти на новый путь
// значит выбросить заполненный людьми конфиг и уехать на дефолтах — ровно та
// поломка, от которой этот код и защищает. В лог при этом идёт предупреждение
// с обоими путями.
