...
Итоги матча (счёт, убийства, раунды, команда) требуют плагин **Utils от Pisex**
(https://github.com/Pisex/cs2-menus) рядом — тот же, что нужен `cs2-lvl_ranks` и `cs2-vip`. Он
необязателен: без него история подключений и пинг пишутся полностью, а эти колонки
остаются нулями, о чём плагин один раз сообщает в консоль.
