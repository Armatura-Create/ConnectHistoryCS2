...
Минимум — **Metamod:Source 2.0.0-git1460**. 8 сентября 2026 Metamod убрал SourceHook
и заменил его на KHook (`a12f3cd5`), а следом поднял версию API плагинов до 18 и
минимальную версию загрузки (`0cc4e200`). Плагин собран под новый API: на сборках
старше git1460 он не загрузится вовсе, а сборки старше git1322 не знают ещё и
`META_CONVAR_REGISTER`, без которого нет команд.

На Metamod **git1467 и новее** нужен ConnectHistory **3.1.4 или новее**. В git1467
у KHook поменялась сигнатура `RemoveHook`, и 3.1.3, собранный раньше, при выгрузке
плагина или остановке сервера передаёт ядру мусор вместо указателя на функцию.
Новые сборки работают на всём диапазоне от git1460.

Итоги матча (счёт, убийства, раунды, команда) читаются с контроллера игрока, и для
этого плагину нужно ровно одно число, зависящее от версии игры, — смещение системы
сущностей внутри `IGameResourceService`. Оно лежит в `addons/configs/ConnectHistory/gamedata.json`
(по умолчанию Linux 80, Windows 88 — ключ `GameEntitySystem` в gamedata CS2Fixes),
никогда не перезаписывается и правится без пересборки. Если после обновления CS2 итоги
стали нулями, а в консоли `[Gamedata]` ругается на смещение — возьмите свежее значение
и перезапустите сервер. Никаких других плагинов не требуется.
